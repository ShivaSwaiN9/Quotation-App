...
import { cardsArr, companiesArr } from "@/utils/Index";
import axios from "axios";
import CancelIcon from "@mui/icons-material/Cancel";
import CommonCards from "@/components/commonComponents/CommonCards";
...
export default function Home() {
  ...
  const fetchData = async (path: string) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:6567/api/v1/${path}/getall`,
  ...
  const previewPDF = async (compid: string) => {
    try {
      const response = await axios.get(
        `http://localhost:6567/api/v1/quotation/pdf/${compid}`,
        { responseType: "blob" }
      );
  ...
  const handleDelete = async (path: string, id: number) => {
    try {
      await axios.delete(`http://localhost:6567/api/v1/${path}/delete/${id}`);
    ...
    try {
      ...
      await Promise.all(
        selectedData.map(async (data) => {
          ...
          await axios.post(
            "http://localhost:6567/api/v1/customer/create",
            { ...data, compid },
            { withCredentials: true }
          );
